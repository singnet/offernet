...
knitr::include_graphics("pictures/network_before_processes_birdeye.png")
```
...
knitr::include_graphics("pictures/initial_network_close.png")
```
...
knitr::include_graphics("pictures/offernet_graph_schema.png")
```
...
Similarity search is the process which searches similar `items` and connects them with `similarity` links. The `value` of an item is represented as a real number in the range $[0,1]$. Similarity between two items is calculated using the formula $Sim = 1 - abs(value_{i1} - value_{i2})$ which also results in the real number of range $[0,1]$ -- the closer the number to one, the more `similar` items are. 
