...
```{r all-simulation-parameters, fig.cap=paste('Distribution of simulation parameters of all analysed experiments (',Sys.getenv('R_TOTALSIM'),' simulations in total).'), out.width='100%', fig.align='center', echo=FALSE, eval=TRUE}
library(ggplot2)
...
exp16.env <- LoadToEnvironment('~/offernet/docs/experiment-1-run-6/R_data/summary_of_all_experiments.Rdata')
exp17.env <- LoadToEnvironment('~/offernet/docs/experiment-1-run-7/R_data/summary_of_all_experiments.Rdata')
exp18.env <- LoadToEnvironment('~/offernet/docs/experiment-1-run-8/R_data/summary_of_all_experiments.Rdata')
...
all.df <- rbind(all.df, exp16.env$summary.df)
all.df <- rbind(all.df, exp17.env$summary.df)
all.df <- rbind(all.df, exp18.env$summary.df)

Sys.setenv(R_TOTALSIM = length(unique(all.df$simulationId)))
...
run4 <- c('EXP09-25-07-21-KtTayU','EXP09-30-10-25-HycEjd')
...
'EXP10-08-01-22-fA74de',
'EXP10-10-01-42-zrrpbq')
...
run8 <- c(
'EXP10-22-01-35-m9gbIT'
)

set1 <- c(run1,run2,run3)
set2 <- c(run4,run5)
set3 <- c(run6,run7,run8)

all.df$set <- ifelse(all.df$experimentId %in% set1,"1",
                     ifelse(all.df$experimentId %in% set2,"2", 
                            "3"))


#all.df$run <- ifelse(all.df$experimentId %in% run1,"1",
#                     ifelse(all.df$experimentId %in% run2,"2", 
#                            ifelse(all.df$experimentId %in% run3,"3",
#                                   ifelse(all.df$experimentId %in% run4,"4", 
#                                      ifelse(all.df$experimentId %in% run4,"5", 
#                                      "6")
#                                      )
#                                    )
#                                  )
...
byAgentNumber <-
  all.df %>%
  group_by(agentNumber, set) %>%
  summarise(n_distinct(simulationId))
names <- c('agentNumber','set','simulationCount')
...
byAgentNumberPlot <- ggplot(byAgentNumber, aes(x = set, y = simulationCount, fill = agentNumber, label = agentNumber)) +
  geom_bar(stat = "identity") +
  geom_text(size = 3, position = position_stack(vjust = 0.5)) +
  scale_fill_continuous(low="green4", high="brown")+
  geom_text(aes(set, total, label = total, fill = NULL), data = totals, position=position_dodge(width=0.9), vjust=-0.4) +
...
byMaxDistance <-
  all.df %>%
  group_by(maxDistance, set) %>%
  summarise(n_distinct(simulationId))
names <- c('maxDistance','set','simulationCount')
...
byMaxDistancePlot <- ggplot(byMaxDistance, aes(x = set, y = simulationCount, fill = maxDistance, label = maxDistance)) +
  geom_bar(stat = "identity") +
  geom_text(size = 3, position = position_stack(vjust = 0.5)) +
  #scale_fill_continuous(low="green4", high="brown")+
...
byChainLength <-
  all.df %>%
  group_by(chainLength, set) %>%
  summarise(n_distinct(simulationId))
names <- c('chainLength','set','simulationCount')
...
byChainLengthPlot <-ggplot(byChainLength, aes(x = set, y = simulationCount, fill = chainLength, label = chainLength)) +
  geom_bar(stat = "identity") +
  geom_text(size = 3, position = position_stack(vjust = 0.5)) +
...
byWorksMultipliers <-
  all.df %>%
  group_by(randomWorksNumberMultiplier, set) %>%
  summarise(n_distinct(simulationId))
names <- c('randomWorksNumberMultiplier','set','simulationCount')
...
byWorksMultipliersPlot <-ggplot(byWorksMultipliers, aes(x = set, y = simulationCount, fill = randomWorksNumberMultiplier, label = randomWorksNumberMultiplier)) +
  geom_bar(stat = "identity") +
  geom_text(size = 3, position = position_stack(vjust = 0.5)) +
...
byConnectThreshold <-
  all.df %>%
  group_by(similarityConnectThreshold, set) %>%
  summarise(n_distinct(simulationId))
names <- c('similarityConnectThreshold','set','simulationCount')
...
byConnectThresholdPlot <-ggplot(byConnectThreshold, aes(x = set, y = simulationCount, fill = similarityConnectThreshold, label = similarityConnectThreshold)) +
  geom_bar(stat = "identity") +
  geom_text(size = 3, position = position_stack(vjust = 0.5)) +
...
exp11.df <- select(exp11.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles)
all.df <- exp11.df

exp12.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-2-corrected/R_data/summary_of_all_experiments.Rdata')$summary.df
exp12.df <- select(exp12.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles)
...
exp13.df <- select(exp13.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles)
...
exp14.df <- select(exp14.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles)
...
exp15.df <- select(exp15.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles)
...
exp16.df <- select(exp16.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles)
...
exp17.df <- select(exp17.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles)
all.df <- rbind(all.df, exp17.df)

exp18.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-8/R_data/summary_of_all_experiments.Rdata')$summary.df
exp18.df <- select(exp18.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles)
...
exp11.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-1-corrected/R_data/summary_of_all_experiments.Rdata')$summary.df
exp11.df <- exp11.df[which(exp11.df$similarityConnectThreshold != "NA"),]
exp11.df <- select(exp11.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles, graphType)
all.df <- exp11.df

exp12.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-2-corrected/R_data/summary_of_all_experiments.Rdata')$summary.df
exp12.df <- select(exp12.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles, graphType)
all.df <- rbind(all.df, exp12.df)

exp13.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-3-corrected/R_data/summary_of_all_experiments.Rdata')$summary.df
exp13.df <- select(exp13.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles, graphType)
all.df <- rbind(all.df, exp13.df)

exp14.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-4/R_data/summary_of_all_experiments.Rdata')$summary.df
exp14.df <- select(exp14.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles,graphType)
all.df <- rbind(all.df, exp14.df)

exp15.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-5/R_data/summary_of_all_experiments.Rdata')$summary.df
exp15.df <- select(exp15.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles,graphType)
all.df <- rbind(all.df, exp15.df)

exp16.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-6/R_data/summary_of_all_experiments.Rdata')$summary.df
exp16.df <- select(exp16.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles,graphType)
all.df <- rbind(all.df, exp16.df)

exp17.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-7/R_data/summary_of_all_experiments.Rdata')$summary.df
exp17.df <- select(exp16.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles,graphType)
all.df <- rbind(all.df, exp17.df)

exp18.df <- LoadToEnvironment('~/offernet/docs/experiment-1-run-8/R_data/summary_of_all_experiments.Rdata')$summary.df
exp18.df <- select(exp18.df, maxDistance, sum_wallTime_min_total, simulationType, foundCycles, graphType)
all.df <- rbind(all.df, exp18.df)
...
summ.df <- ddply(all.df,~agentNumber+similarityConnectThreshold,summarise,avg_vertices_total=mean(vertices_total),avg_sum_wallTime_min_total=mean(sum_wallTime_min_total))

ggplot(summ.df, aes(x=avg_vertices_total, y=avg_sum_wallTime_min_total, color=similarityConnectThreshold)) + 
  geom_point( ) + 
  geom_smooth(method="loess", se=F) + 
  scale_x_continuous(labels=fancy_scientific) +
