...
run5 <- c('EXP10-02-01-04-YQvrhC')

all.df$run <- ifelse(all.df$experimentId %in% run1,"Run1",
                     ifelse(all.df$experimentId %in% run2,"Run2", 
                            ifelse(all.df$experimentId %in% run3,"Run3",
                                   ifelse(all.df$experimentId %in% run4,"Run4", 
                            "Run5"))))
...
First, we run a simple linear regression on all results to obtain an indication of which parameters from parameter set may be more important and significant. It should be noted that from the strictly statistical point this analysis is not very sound, because the parameter space is not very well distributed (see above). Nevertheless, the linear regression summary seem to give some useful information, which of course has to be further tested and updated when as soon as more data is available.
...
fitCV <- lm(sum_wallTime_min_total ~ agentNumber +  vertices_total + edges_total  +  chainLength + maxDistance + similaritySearchThreshold + similarityConnectThreshold + randomWorksNumberMultiplier, data=all.df[which(all.df$simulationType == 'Centralized'),])
fitDV <- lm(sum_wallTime_min_total ~ agentNumber  + vertices_total + edges_total  + chainLength + maxDistance + similaritySearchThreshold + similarityConnectThreshold + randomWorksNumberMultiplier, data=all.df[which(all.df$simulationType == 'Decentralized'),])
