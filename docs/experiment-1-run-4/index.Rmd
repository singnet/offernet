...
selectedExperiments <- c('EXP09-25-07-21-KtTayU','EXP09-30-10-25-HycEjd')
descriptions <- c('agentNumbers=[50, 100, 200, 400, 800] : chainLengths=[5, 10, 20] : randomWorksNumberMultipliers=[1, 2] : maxDistances=[5, 10, 30] : similaritySearchThresholds=[1] : similarityConnectThresholds=[0.9] : graphTypes=[random, smallWorld] ; Crashed somewhere on 400 agents in be start of randomWorksNumberMultiplier 2.','agentNumbers=[400] : chainLengths=[5] : randomWorksNumberMultipliers=[2] : maxDistances=[5, 10, 30] : similaritySearchThresholds=[1] : similarityConnectThresholds=[0.9] : graphTypes=[random, smallWorld]; Rerunning from the place where crashed;')
```
...
for (experimentId in selectedExperiments) {
  ...
  result <- Search(index="filebeat-*", source=source, body=query, asdf=TRUE, size=30000)$hits$hits
  ...
  #simulations.df <- simulations.df[which(simulations.df$simulationId != "SIM09-27-08-30-rp7cU3--CV"), ]
  #simulations.df <- simulations.df[which(simulations.df$simulationId != "SIM09-27-10-35-fkjari--CV"), ]
  #simulations.df <- simulations.df[which(simulations.df$simulationId != "SIM09-27-03-25-FPvUfB--CV"), ]
  #simulations.df <- simulations.df[which(simulations.df$simulationId != "SIM09-27-07-26-jn7Iyx--CV"), ]
