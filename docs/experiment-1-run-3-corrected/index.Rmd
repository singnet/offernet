...
selectedExperiments <- c('EXP09-05-02-15-an9jYp','EXP09-05-11-49-gcWcHs','EXP09-07-02-54-hATxwM')
descriptions <- c('agentNumbers=[400, 500, 600] : chainLengths=[10] : randomWorksNumberMultipliers=[1] : maxDistances=[10] : similaritySearchThresholds=[1] : similarityConnectThresholds=[0.9, 0.93, 0.96, 0.99] : message=[pre-generated smallWorld graph of agents with known diameters < 10]','Same as above, only agentNumbers=[700, 800]', 'Same as above, agentNumbers=[700, 800], randomWorksNumberMultipliers=[2]')
```
...
summary.df$graphType <- 'smallWorld'
...
fitCV <- lm(sum_wallTime_min_total ~ agentNumber +  vertices_total + edges_total  +  chainLength + randomWorksNumberMultiplier, data=summary.df[which(summary.df$simulationType == 'Centralized'),])
fitDV <- lm(sum_wallTime_min_total ~ agentNumber  + vertices_total + edges_total  + chainLength + randomWorksNumberMultiplier, data=summary.df[which(summary.df$simulationType == 'Decentralized'),])
fit <- lm(sum_wallTime_min_total ~ agentNumber + vertices_total + edges_total + factor(simulationType) + chainLength + randomWorksNumberMultiplier, data=summary.df)
